...
type: note
permalink: tacticum/04-sessions/avtonomnyi-progon-rag-2-polnyi-prokhod-glubokaia-proverka-mandat
tags:
- helm
- rag2
- mcp
...
## Треки в работе
- Воркер `cross-rerank` (пункт 4): кросс-корпусный реранк за флагом `rag2_cross_rerank_enabled` (default off, fail-soft) + тесты. Хендофф → `00-Inbox/cross-rerank-SUMMARY.md`. После: A/B на живых данных → если качество лучше, включить.
