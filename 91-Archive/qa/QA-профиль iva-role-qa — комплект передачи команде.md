...
- iva-role-qa
- onboarding
permalink: tacticum/91-archive/qa/qa-profil-iva-role-qa-komplekt-peredachi-komande
---

# QA-профиль `iva-role-qa` — комплект передачи команде

> **АРХИВ (2026-07-25).** Это предыдущая редакция документа передачи (10:17 того же дня). Через несколько часов вышла исправленная редакция — [[iva-role-qa — установка и работа (для команды)]]; именно она отдана команде и является единственной актуальной. Всё содержательное отсюда (состав профиля, чего он не делает, провижн установки, критерии первого прогона, негативный тест секретов, открытые уточнения) перенесено туда с исправленными фактами. Сюда перенесён без изменений содержания, чтобы не потерять исходник.
> ⚠️ **Устаревшие факты, не отправлять команде:** здесь репозиторий назван `one-web` (Angular) и стек «Playwright/Selenium» — по факту это `one-web-kmp` (KMP, canvas-рендер) и pytest + Playwright, исправлено 2026-07-24 после проверки самого профиля. Версия профиля `0.4.0` устарела (в проде роль 0.5.1). Формулировка «пока web-only» неверна — корректно «пока один стек: pytest + Playwright canvas».
