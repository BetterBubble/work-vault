...
## Зачем
ТЗ#1 (дизайн-процесс Figma↔код) закрыто и выкачено, но часть работ по нему **сознательно вынесена
за наш контур** — их делают другие команды. Эта заметка держит остаток в одном месте, чтобы он не
растворился: работа закрыта у нас, но не закрыта вообще, и через месяц об этом спросят.

## Как работает
Список ниже — не задачи в работе, а **передача**: по каждому пункту сказано, что именно осталось,
кто владелец и от чего это зависит. Пункт уходит отсюда, когда владелец подтвердил, что взял.

## Что конкретно сделано
- Остаток вычленен из ТЗ#1 и разложен по пяти направлениям (миграция iva-one, серверная дизайн-система
  в iva-core, словарь v2 с авто-пересборкой и CI, незаполненные ключи Figma, внутренние ADR/процесс).
- По каждому названы владелец и зависимость, чтобы передача была адресной, а не «кому-нибудь».
- Само ТЗ#1 при этом закрыто и выкачено в прод-каталог 25.07 — см. [[tz1-figma-ds-process]].
...
## Что осталось
Весь список выше и есть остаток — по каждому пункту работа у владельца, не у нас. Снимать пункт
можно только после подтверждения владельца, что он взял; молчание подтверждением не считается.

## Блокеры
- Пункты 1–4 зависят от команд вне нашего контура: пока владельцы не подтвердили приём, движения по ним не будет, и это ожидаемо, а не сбой. На 2026-07-26 подтверждений нет.

## Как проверяли (доказательства)
- Состав остатка сверен с ТЗ#1 построчно при закрытии работы 25.07: в остаток попало ровно то, что
  не влезло в предел шаблонов и требует чужих репозиториев.
- Проверять здесь больше нечего до передачи: это список обязательств, а не работающий код. Момент
  проверки наступит, когда владелец возьмёт пункт и закроет его у себя.

## Ход / решения
- 2026-07-25 — остаток вынесен отдельной заметкой при закрытии ТЗ#1. Причина: закрывать ТЗ, оставив
  часть работ безымянными, значит потерять их — через месяц никто не вспомнит, что они были в объёме.
